...
///////////////////////////////////////////////////////////
// HyunVcfFileException class 
// Creates a exception with printf style outputs
//////////////////////////////////////////////////////////
class HyunVcfFileException : public std::exception {
 public: 
  HyunVcfFileException(const char* format, ... ) {
    va_list args;
    va_start (args, format);
    msg.printf("HyunVcfFileException: ");
    msg.vprintf(format, args);
    va_end(args);
  }

  ~HyunVcfFileException() throw() {}
...
class HyunVcfFile {
 public:
  ...
  VcfMarker* getLastMarker(int nFromHead) { 
	...
	throw HyunVcfFileException("HyunVcfFile::getLastMarker() - Null pointer exception, nHead=%d, nFromHead=%d, nBuffers=%d", nHead, nFromHead, nBuffers);
      }
      */
      return vpVcfMarkers[(nHead-nFromHead-1+nBuffers) % nBuffers]; 
    }
    else {
      throw HyunVcfFileException("HyunVcfFile::getLastMarker() - Index out of bound. nFromHead = %d, nBuffers = %d", nFromHead, nBuffers);
    }
  }
...
// BED file format 
class BedFile : public HyunVcfFile {
 public:
  IFILE iBimFile;
  IFILE iFamFile;
  String sRefFile;
