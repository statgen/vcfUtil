...
uint64_t VcfHelper::str2TwoBits(const char* s, int len) {
  if ( len > 32 ) {
    throw HyunVcfFileException("Cannot encode sequences of length > %d",len);
  }
  uint64_t ret = 0;
  for(int i=0; i < len; ++i) {
...
int VcfHelper::chromName2Num(const String & chr) {
  ...
  throw HyunVcfFileException("Cannot recognize chromosome %s",chr.c_str());
}
...
void HyunVcfFile::openForRead(const char* filename, int nbuf) {
  reset();
  
  iFile = ifopen(filename,"rb");
  if ( iFile == NULL ) {
    throw HyunVcfFileException("Failed opening file %s - %s",filename, strerror(errno));
  }
  nBuffers = nbuf;
  nNumMarkers = 0;
  nHead = 0;
...
void HyunVcfFile::parseMetaLine() {
...
// copied from Matthew Flickenger/Snyder
void HyunVcfFile::verifyMetaLines() {
  if ( asMetaKeys.Length() < 1 ) {
    //throw HyunVcfFileException("No meta ## lines found. ##format is required");
  }
  if ( asMetaKeys.Find("format") == -1 && asMetaKeys.Find("fileformat") == -1 ) {
    //throw HyunVcfFileException("Required ##format line not found");
  }
}

// specific to upgrade from VCF 3.3 to VCF 4.0
void HyunVcfFile::upgradeMetaLines() {
...
void HyunVcfFile::parseHeader() {
  if ( isHeaderLine() ) {
    ...
    throw HyunVcfFileException("Header line is not found : #CHROM...");
  }
}

void HyunVcfFile::verifyHeaderLine() {
   String expected[9] = {"#CHROM", "POS","ID","REF",
      "ALT","QUAL","FILTER","INFO", "FORMAT"};
   for(int i=0; i<8; i++) {
      if(lineTokens[i] != expected[i]) {
	throw HyunVcfFileException("Bad header: expected %s, found %s",expected[i].c_str(),lineTokens[i].c_str());
      }
   }
   if (lineTokens.Length() > 9) {
      if(lineTokens[8] != expected[8]) {
	throw HyunVcfFileException("Bad header: expected %s, found %s, token Length == %d",expected[8].c_str(),lineTokens[8].c_str(),lineTokens.Length());
      }
   }
}

bool HyunVcfFile::isMetaLine() {
   return line[0] == '#' && line[1] == '#';
}

bool HyunVcfFile::isHeaderLine() {
   return line[0] == '#' && line[1] == 'C';
}

int HyunVcfFile::getMetaCount() {
   return asMetaKeys.Length();
}

int HyunVcfFile::getMetaIndex(const String& meta) {
   return asMetaKeys.Find(meta);
}

bool HyunVcfFile::hasMetaKey(const String& meta) {
   return asMetaKeys.Find(meta)!=-1;
}

String HyunVcfFile::getMetaKey(int offset) {
   return asMetaKeys[offset];
}

String HyunVcfFile::getMetaValue(int offset) {
   return asMetaValues[offset];
}
String HyunVcfFile::getMetaValue(const String& meta) {
   return asMetaValues[getMetaIndex(meta)];
}

String HyunVcfFile::getMetaValue(int offset, const String& ifmissing) {
   if (offset > -1) {
...
String HyunVcfFile::getMetaValue(const String& meta, const String& ifmissing) {
   return getMetaValue(getMetaIndex(meta), ifmissing);
}

int HyunVcfFile::getSampleCount() {
  return static_cast<int>(vpVcfInds.size());
}

int HyunVcfFile::getSampleIndex(const String& name) {
  for(int i=0; i < (int)vpVcfInds.size(); ++i) {
...
bool HyunVcfFile::hasSample(String name) {
   return getSampleIndex(name) != -1;
}

String HyunVcfFile::getSampleID(int offset) {
  if ( offset < (int)vpVcfInds.size() ) {
    return vpVcfInds[offset]->sIndID;
  }
  else {
    throw HyunVcfFileException("HyunVcfFile::getSampleID(%d) - Array out of bound (>%d)",offset,(int)vpVcfInds.size());
  }
}
...
void BedFile::openForRead(const char* bedFile, const char* bimFile, const char* famFile, const char* refFile, int nbuf) {
  ...
  if ( iFile == NULL ) {
    throw HyunVcfFileException("Failed opening file %s - %s",bedFile,strerror(errno));
  }
  ...
  for(int i=0; i < 3; ++i) {
    if ( firstThreeBytes[i] != magicNumbers[i] ) {
      throw HyunVcfFileException("The magic numbers do not match in BED file %s",bedFile);
    }
  }
  ...
  while( 1 ) {
    ...
    if ( tokens.Length() < 5 ) {
      throw HyunVcfFileException("Less then 5 columns are observed in FAM file");
    }
...
bool HyunVcfFile::iterateMarker() {
  if ( readLine() <= 0 ) { 
    bEOF = true;
    return false; 
  }
  ...
  if ( nBuffers == 0 ) {
    ...
    if ( pMarker == NULL ) {
      throw HyunVcfFileException("nHead = %d, nBuffers = %d, pMarker is NULL",nHead,nBuffers);
    }
  ...
  catch (HyunVcfFileException exc) {
    // add the line number to the error message
    throw HyunVcfFileException(exc.msg + " See line " + nNumLines + ".");
  }
  return true;
}
...
void VcfMarker::setInfo(const String& s, bool upgrade) {
  ...
  if ( upgrade ) {
    ...
    if ( asInfoKeys[2].Compare("MAF") == 0 ) {
      ...
      throw HyunVcfFileException("VcfMarker::setInfo() : Cannot upgrade info field entry %s",s.c_str());
    }
...
void VcfMarker::setSample(int sampleIndex, const String& sampleValue, bool parseGenotypes, bool parseDosages, bool parseValues, int minGD, int minGQ) {
  ...
  if ( (sampleValue.Compare("./.") == 0) || ( sampleValue.Compare(".") == 0 ) ) {
    ...
    if ( tmpTokens.Length() != asFormatKeys.Length() ) {
      throw HyunVcfFileException("# values = %s do not match with # fields in FORMAT field = %d at sampleIndex = %d",sampleValue.c_str(),asFormatKeys.Length(),sampleIndex);
    }
	...
	if ( ( GD < minGD ) || ( GQ < minGQ ) ) {
	  ...
	  if ( sepPos >= 0 ) {
		...
		throw HyunVcfFileException("Cannot parse the genotype field " + tmpTokens[GTindex]);
	      }
	    }
	  }
...
bool BedFile::iterateMarker() {
  ...
  if ( lineTokens.Length() < 6 ) {
    throw HyunVcfFileException("BIM file has only %d columns - %s",lineTokens.Length(),lineTokens[0].c_str());
  }
  ...
  try {
	...
	markerIndex = genomeSequence.getGenomePosition( strChr.c_str(), pMarker->nPos );
	if ( markerIndex == INVALID_GENOME_INDEX ) {
	  throw HyunVcfFileException("Cannot parse chromosome name "+lineTokens[0]+" in BIM file");
	}
      }
    }
	...
	break;
      default:
	throw HyunVcfFileException("Error in parsing genotypes");
      }
    }
...
char BedFile::determineAltBase(char refBase, char a1, char a2) {
	...
	if ( f1 == refBase ) {
      ...
      //throw HyunVcfFileException("The ref/alt allele does not match to reference genome");
      return '.';
    }
}

void HyunVcfFile::printVCFHeader(IFILE oFile) {
...
void HyunVcfFile::printVCFHeaderSubset(IFILE oFile, std::vector<int>& subsetIndices) {
  //fprintf(stderr,"foo\n");
  for(int i=0; i < getMetaCount(); ++i) {
...
void HyunVcfFile::printBEDHeader(IFILE oBedFile, IFILE oFamFile) {
  for(int i=0; i < getSampleCount(); ++i) {
    ...
    case VcfInd::FEMALE:
      ...
      break;
    default:
      throw HyunVcfFileException("Unrecognized value for gender");
      break;
    }
    ifprintf(oFamFile,"\t-9\n");
  }
...
void VcfHelper::printArrayDoubleJoin(IFILE oFile, const StringArray& arr1, const StringArray& arr2, const char* sep1, const char* sep2, const char* empty) {
  ...
  if ( len1 == len2 ) {
    ...
    throw HyunVcfFileException("Inconsistency between arr1.Length() == %d and arr2.Length() == %d", len1, len2);
  }
}
